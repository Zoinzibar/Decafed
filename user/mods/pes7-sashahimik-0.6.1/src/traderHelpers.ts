...
export class TraderHelper
{
    ...
    public addTraderToDb(assorty: any, traderDetailsToAdd: any, dialoguey: any, tables: IDatabaseTables, jsonUtil: JsonUtil): void
    {
        // Add trader to trader table, key is the traders id
        tables.traders[traderDetailsToAdd._id] = {
            assort: assorty as ITraderAssort, // assorts are the 'offers' trader sells, can be a single item (e.g. carton of milk) or multiple items as a collection (e.g. a gun)
            ...
            dialogue: dialoguey as Record<string, string[]>,
            questassort: {
                started: {},
                success: {},
                fail: {}
    ...
    public addBarterItem(traderId: string, tables: IDatabaseTables, sellableItemId: string, count: number, itemTpl: string): void
    {
        const barterSchema = tables.traders[traderId].assort.barter_scheme;

        if (!barterSchema[sellableItemId]?.length) 
        {
            barterSchema[sellableItemId] = [
                [
                    {
                        count: count,
                        _tpl: itemTpl
        ...
        {
            //logger.log(`[${sellableItemId}] Barter Found adding new item to it: `, barterSchema[sellableItemId])
            barterSchema[sellableItemId][0].push({
                count: count,
                _tpl: itemTpl
            });
        }
